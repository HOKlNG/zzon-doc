...
- **v0.8.3 — 뷰어 버그픽스 · 저작 정책** : 노드를 클릭해도 상세 사이드바가 열리지 않던 버그(포인터 캡처가 클릭을 svg 루트로 리타깃)와 플로우 해제 후 강조 잔상이 남던 버그를 수정했습니다. 저작 정책도 교정했습니다 — 설명 문장은 캔버스가 아니라 노드 `description`(우측 사이드바)에 담고, 구조 장에도 대표 플로우 1~2개를 기본 포함합니다.
