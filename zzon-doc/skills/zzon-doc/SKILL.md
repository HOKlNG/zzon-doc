...
- **포괄적 요청** — "이 프로젝트 그려줘" / "전체 구조" → **개요 한 장으로 때우지 말 것.** 깊이를 재서 보통 **여러 장짜리 문서 세트**를 제안한다:
  1. **깊이를 개수로 파악한다.** 서비스·모듈 수, 핵심 기능·유스케이스 수, 도메인/바운디드 컨텍스트, 테이블 수, 외부 연동·비동기 흐름. ("기능 N개, 테이블 M개" 수준까지 센다 — 단순 나열 금지.)
  2. **규모에 맞게 문서 세트를 구성한다 — 개요 1장이 기본값이 아니다:**
     - 전체 인프라 / 시스템 컨텍스트 — 1장
     - **기능·도메인별 data-flow는 기능마다 1장** (결제·검색·인증·알림 …). 기능이 많으면 문서도 많아진다.
     - 데이터 ERD — 1장(테이블 많으면 도메인별로 분할)
     - 해당 시 `.claude` agent-topology, 배포/CI 등
  3. **추천을 "문서 목록 + 장수"로 제시하고 고르게 한다.** 예: "서비스 4·핵심기능 6·테이블 12 확인. **추천(상세) 9장: ①전체 인프라 ②~⑦ 기능별 플로우 6장 ⑧⑨ 도메인 ERD 2장.** 가볍게는 3장(개요+ERD+대표 플로우). 어느 쪽으로 갈까, 더 좁힐까?"
  4. 합의한 목록을 **전부** 그려 통합 문서(build-docs)로 묶는다.
